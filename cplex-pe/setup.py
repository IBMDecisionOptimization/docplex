...
# We can't import through cplex_pe package because __init__.py
# tries to load the native libraries which don't exist yet during setup
sys.path.insert(0, str(Path(__file__).parent / 'src' / 'cplex_pe'))
...
def get_platform_libs():
    ...
    (src/cplex_pe/) so setuptools includes them as package data.
    """
    library_path = get_library_path()

    # Absolute path to the platform library directory
    libs_path = Path(__file__).parent / 'src' / 'cplex_pe' / library_path
    ...
    # Return paths relative to src/cplex_pe/ for package_data
    pkg_root = Path(__file__).parent / 'src' / 'cplex_pe'
    return [str(f.relative_to(pkg_root)) for f in lib_files]


setup(
    package_data={
        "cplex_pe": get_platform_libs()
    },
)
