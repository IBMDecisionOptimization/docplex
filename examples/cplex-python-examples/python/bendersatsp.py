...
# ---------------------------------------------------------------------------
r"""
Example bendersatsp.py solves a flow MILP model for an Asymmetric
...
branch-and-cut process via the cut callback classes
LazyConstraintCallback and UserCutCallback. The cut callbacks add to the
master ILP violated Benders' cuts that are found by solving the worker
LP.
...
In this case, Benders' cuts are treated as lazy constraints through the
class LazyConstraintCallback. In addition, Benders' cuts are also treated
as user cuts through the class UserCutCallback.
