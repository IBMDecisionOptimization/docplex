...
# ---------------------------------------------------------------------------
"""
Given a set of warehouses that each have a capacity, a cost per unit
stored, and a minimum usage level, this example find an assignment of
items to warehouses that minimizes total cost.  The minimum usage levels
are enforced by using semi-integer variables.
...
def warehouse():
    ...
    if sol.is_primal_feasible():
        ...
        for i in range(nbwhouses):
            print("Warehouse %d capacity = %d" %
                  (i, sol.get_values(capvars[i])))
            print("  loads:", end=' ')
            for j in range(nbloads):
