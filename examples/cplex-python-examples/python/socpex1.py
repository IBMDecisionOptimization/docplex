...
# ---------------------------------------------------------------------------
"""
Solve a second order cone program to optimality, fetch the dual values
and test that the primal and dual solution vectors returned by CPLEX
satisfy the KKT conditions.
The problems that this code can handle are second order cone programs in
standard form. A second order cone program in standard form is a problem
of the following type (c' is the transpose of c):
  min c1'x1 + ... + cr'xr
    A1 x1 + ... + Ar xr = b
    xi in second order cone (SOC)
where xi is a vector of length ni. Note that the different xi are
orthogonal. The constraint "xi in SOC"
  for xi=(xi[1], ..., xi[ni]) is
      xi[1] >= |(xi[2],...,xi[ni])|
where |.| denotes the Euclidean norm. In CPLEX such a constraint is
formulated as
  -xi[1]^2 + xi[2]^2 + ... + xi[ni]^2 <= 0
   xi[1]                              >= 0
   xi[2], ..., xi[ni] free
Note that if ni = 1 then the second order cone constraint reduces to
xi[1] >= 0.
"""
...
def checkkkt(c, cone, tol):
    ...
    dslack = dict()
    for key, value in getsocpconstrmultipliers(c, dslack).items():
        pi[key] = value
    for (q, v) in zip(
    ...
    for n in c.linear_constraints.get_names():
        if c.linear_constraints.get_senses(n) == 'L' and pi[n] < -tol:
            print("Dual multiplier ", pi[n],
                  " for <= constraint ", n, " not feasible.", file=sys.stderr)
            return False
    ...
    for n in c.variables.get_names():
        if cone[n] == NOT_IN_CONE and dslack[n] < -tol:
            print("Dual multiplier for ", n, " is not feasible: ",
                  dslack[n], file=sys.stderr)
            return False
    ...
    for n in c.linear_constraints.get_names():
        if fabs(s[n]) > tol and fabs(pi[n]) > tol:
            print("Invalid complementary slackness for ", n,
                  ": ", s[n], " and ", pi[n], file=sys.stderr)
            return False
    for n in c.quadratic_constraints.get_names():
        if fabs(s[n]) > tol and fabs(pi[n]) > tol:
            print("Invalid complementary slackness for ", n,
    ...
    for n in c.variables.get_names():
        if cone[n] == NOT_IN_CONE:
            if fabs(x[n]) > tol and fabs(dslack[n]) > tol:
                print("Invalid complementary slackness for ", n,
                      ": ", x[n], " and ", dslack[n], file=sys.stderr)
                return False
...
def socpex1():
    ...
    if not checkkkt(c, cone, TESTTOL):
        print("Testing of KKT conditions failed.", file=sys.stderr)
    else:
