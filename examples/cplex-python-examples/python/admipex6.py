...
# ---------------------------------------------------------------------------
"""
Start the solution of the root relaxation in a MIP model from an existing
primal solution with the solve callback.
...
class UserSolve(SolveCallback):
    ...
    def __call__(self):
        ...
        if self.times_called == 1:
            self.set_start(cplex.SparsePair(list(range(self.get_num_cols())),
                                            self.relx))
...
def admipex6(filename):
    try:
        ...
        if (c.variables.get_num_semiinteger() +
            ...
            print("Not for problems with semi-continuous or "
        ...
        clone.solve()
        print("Solution status = ", clone.solution.get_status(), ":", end=' ')
