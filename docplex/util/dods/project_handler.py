    ...
    def __getitem__(self, y):
        if isinstance(y, str):
            ...
            if item is None:
                ...
                with env.get_input_stream(file) as in_stream:
                    if env.interpret_df_type is True:
                        df = pandas.read_csv(in_stream, encoding='utf8')
                    else:
                        df = pandas.read_csv(in_stream, encoding='utf8', dtype=str)
                    dict.__setitem__(self, y, df)
                    return df
            else:
                return item
        else:
