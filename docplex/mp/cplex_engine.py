...
class CplexEngine(IEngine):
    ...
    @classmethod
    def _compute_lp_probtype_set(cls, cpx_cst):
        return frozenset([cpx_cst.CPXPROB_LP, cpx_cst.CPXPROB_FIXEDMILP, cpx_cst.CPXPROB_FIXEDMIQP])

    def solved_as_lp(self):
        # INTERNAL
        cpx = self._cplex
        if self.lp_probtypes is None:
            self.lp_probtypes = self._compute_lp_probtype_set(self.cpx_cst)
    ...
    def __init__(self, mdl, **kwargs):
        ...
        # default: look in system
        coslocation = context.cos.location if context is not None else None
    ...
    def get_all_cuts(self, mdl):
        if self._cplex._is_MIP():
            cut_stats = {}
            cut_type_instance = CutType()
            for ct in cut_type_instance:
                num = self._cplex.solution.MIP.get_num_cuts(ct)
                cutname = cut_type_instance[ct]
                cut_stats[cutname] = num
            cut_stats = dict(sorted(cut_stats.items(), key=lambda item: item[1], reverse = True))
            return cut_stats
        return {}

    def get_sensitivity(self, dvars):
        sensitivity = self._cplex.solution.sensitivity
        ret = []
        for dv in dvars:
            # cplex Python requires indices
            dvx = dv.index
            lbs = sensitivity.lower_bounds([dvx])
            ubs = sensitivity.upper_bounds([dvx])
            objs = sensitivity.objective([dvx])
            ret.append( (lbs, ubs, objs) )
        return ret
    ...
    def create_block_linear_constraints(self, linct_seq):
        ...
        if self.procedural:
            ret_add = cpx_adapter.fast_add_linear(self._cplex, cpx_linexprs, cpx_sense_string, cpx_rhss, cpx_names,
    ...
    def create_batch_logical_constraints(self, logcts, is_equivalence):
        ...
        if self.procedural and cpx_adapter.chbmatrix:
            ...
            if hasattr(cpx_indicators, "add_batch"):
                ...
                cpx_indtypes = [self.CPX_IF_ONLYIF_TYPE] * nb_logicals if is_equivalence else [
        ...
        try:
            ...
            if annotated_by_scope:
                ...
                if benders_idx < 0:
                    ...
                    benders_idx = cpx_anno.add(self.cpx_cst.CPX_BENDERS_ANNOTATION, 0)  # quid of defval?
        ...
        if lex_timelimits is not None or lex_mipgaps is not None:
            ...
            if lex_timelimits is not None:
                for paramIdx, timelimit in enumerate(lex_timelimits):
                    paramsets[paramIdx].add(self.cpx_cst.CPX_PARAM_TILIM, timelimit)
    ...
    def populate(self, **kwargs):
        ...
        if not populate_sol:
            ...
            for p in range(numsol):
                ...
                sol = mdl.new_solution(var_value_dict={dv: solpvals[dv.index] for dv in mdl.iter_variables()},
                                       name=sol_name,
