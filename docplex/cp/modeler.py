...
from docplex.cp.expression import CpoSequenceVar
...
def _no_duplicate(x):
    """ Check that each element is present in a single copy"""
    s = set([])
    for v in x.value:
        assert not s.__contains__(v), "Variable: {} appears twice in array".format(v)
        s.add(v)

def _check_allowed_seq_arrays(seq, a):
    """ Check that array of interval-variables a is allowed for sequence-variable seq"""
    sar = seq.get_interval_variables()
    ss = set(sar)
    sa = set([])
    for v in a.value:
        assert ss.__contains__(v), "Sequence-variable {} does not contain the Interval-Variable {}".format(seq, v)
        assert not sa.__contains__(v), "Variable: {} appears twice in array".format(v)
        sa.add(v)
...
def inferred(arr, *args):
    """ Returns an expression stating that variables must not be inferred by the solver.

    Returns an expression which identifies certain variables as having their
    values inferred by the values taken on by non-inferred variables.
    The returned constraint expression can be added to a model to tell CP Optimizer
    that when all non-inferred variables (those which are included in no *inferred*
    constraint instances) are fixed, then all variables in *arr* will have their
    values determined, typically through equality constraints which are used to compute
    their values from the values of other variables.
    Informally, one might say that the variables so marked are not "decision"
    variables of the model.

    The returned expression should be used in models having only integer variables and
    where the "Neighborhood" search type is not being used.  When used in this way,
    CP Optimizer uses *inferred* as a *hint* to improve its performance.

    Note: No errors occur should CP Optimizer not be able to easily determine the
    value of a variable in *arr* after all non-inferred variables are
    fixed.  CP Optimizer will silently consider those as normal decision variables
    in this case.

    Note: Another common way of representing calculations which are not decisions
    in a model is to use expressions to calculate derived values from the values of
    decision variables. *inferred* can be useful when it is impossible (or complicated)
    to represent a calculation in a "loop-free" way.
    This is common when a global constraint would maintain the values, but
    any expression needs to be created *before* using the global constraint.
    As an example, the *inverse(IloIntExpr[], IloIntExpr[])* constraint takes
    two arrays of variables *a* and *b* and maintains a one-to-one
    correspondance between the values of the variables: when the variables in (say)
    *a* are fixed, then the variables in *b* can be easily calculated.
    Here, we may consider *b* to be inferred from *a* (or indeed, that
    *a* is inferred from *b*).
    ...
    if isinstance(arr, (list, tuple)):
       arr = _convert_arg(arr, "arr", Type_IntVarArray)
    if not ((isinstance(arr, CpoExpr) and arr.is_kind_of(Type_IntVar)) or
            (isinstance(arr, CpoValue) and arr.is_kind_of(Type_IntVarArray))):
       raise CpoException("Invalid parameters {}".format(arr))
    r = CpoFunctionCall(Oper_inferred, Type_Constraint, (arr,))
    return r


def sub_circuit(next, *args):
    """ Creates and returns a circuit-imposing constraint in a model.

    Given an array of *n* successor variables *next*, this constraint imposes the *next*
    variables to all have values in *[0, n-1]* and also imposes that all of their values
    are different.  We can understand *next[i]=j* as meaning that node *j* is immediately
    after node *i*.
    Additionally, this constraint ensures that at most one circuit (a path which forms a
    loop) of length at least two can exist (called the "main" circuit) and all nodes *not*
    involved in this circuit have *next[i] = i*. We can see that if *next[i] = j*, where
    *i* and *j* are *different* values, then nodes *i* and *j* are both on the main circuit.
    Any node *i* where *next[i] = i* is not on the main circuit, so to ensure that a particular
    node *i* appears on the main circuit, one can impose a constraint *next[i] != i*.

    Note: By symmetry, one may also consider the *next* parameter
    to represent the predecessor nodes rather than successor nodes.
    The *inverse(IntExpr[], IntExpr[])* constraint can be used to
    maintain an array of predecessor variables *pred* via
    *cp.inverse(next, pred)*.
    ...
    if args:
        next = [next]
        next.extend(args)
    elif isinstance(next, CpoExpr) and next.is_kind_of(Type_IntVar):
        next = [next]
    if isinstance(next, (list, tuple)):
        next = _convert_arg(next, "next", Type_IntVarArray)
    if not ((isinstance(next, CpoValue) and next.is_kind_of(Type_IntVarArray))):
       raise CpoException("Invalid parameters next={}, args={}".format(next, args))
    return CpoFunctionCall(Oper_sub_circuit, Type_Constraint, (_convert_arg(next, "next", Type_IntExprArray),))
...
def conditional(c, et, ef):
    ...
    """
    if is_bool(c):
        return _convert_arg(et, "et", Type_IntExpr) if c else _convert_arg(ef, "ef", Type_IntExpr)
...
def same_sequence(seq1, seq2, array1=None, array2=None):
    ...
    assert len(seq1) == len(ar1.value), "'seq1' and 'array1' should have the same length"
    assert len(seq2) == len(ar2.value), "'seq2' and 'array2' should have the same length"
    _check_allowed_seq_arrays(seq1, ar1)
    _check_allowed_seq_arrays(seq2, ar2)
    return CpoFunctionCall(Oper_same_sequence, Type_Constraint, (seq1, seq2, ar1, ar2))
...
def same_common_subsequence(seq1, seq2, array1=None, array2=None):
    ...
    If no interval variable array is specified as argument, the mapping between interval variables
    of the two sequences is given by the order of the interval variables in the arrays *array1* and
    *array2* used in the definition of the sequences.

    If interval variable arrays *array1* and *array2* are used, these arrays define the mapping
    between interval variables of the two sequences. Those two arrays should be of the same size.
    Array *array1* must only contain interval variables of sequence *seq1* without any duplicate and
    ...
    Returns:
        Constraint expression
    """
    cposeq1 = _convert_arg(seq1, "seq1", Type_SequenceVar)
    cposeq2 = _convert_arg(seq2, "seq2", Type_SequenceVar)
    if array1 is None:
        assert array2 is None, "Mapping arrays 'array1' and 'array2' should be both given or ignored"
        return CpoFunctionCall(Oper_same_common_subsequence, Type_Constraint, (cposeq1, cposeq2))

    ar1 = _convert_arg(array1, "array1", Type_IntervalVarArray)
    ar2 = _convert_arg(array2, "array2", Type_IntervalVarArray)
    _check_same_size_arrays(ar1, ar2)
    _check_allowed_seq_arrays(cposeq1, ar1)
    _check_allowed_seq_arrays(cposeq2, ar2)
    return CpoFunctionCall(Oper_same_common_subsequence, Type_Constraint, (cposeq1, cposeq2, ar1, ar2))
